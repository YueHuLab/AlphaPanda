...
from AlphaPanda.modules.common.geometry import construct_3d_basis, global_to_local, get_backbone_dihedral_angles
from AlphaPanda.modules.common.layers import AngularEncoding
from AlphaPanda.utils.protein.constants import BBHeavyAtom, AA


class ResidueEmbedding(nn.Module):

    #def __init__(self, feat_dim, max_num_atoms,esm_res_dim=1280,max_aa_types=22):
    def __init__(self, feat_dim, max_num_atoms,max_aa_types=22):
        ...
        #self.esm_res_dim = esm_res_dim
        #huyue
        #infeat_dim = feat_dim + (self.max_aa_types*max_num_atoms*3) + self.dihed_embed.get_out_dim(3) + feat_dim+self.esm_res_dim
        infeat_dim = feat_dim + (self.max_aa_types*max_num_atoms*3) + self.dihed_embed.get_out_dim(3) + feat_dim
        self.mlp = nn.Sequential(
            nn.Linear(infeat_dim, feat_dim * 2), nn.ReLU(),#huyue
            #nn.Linear(infeat_dim, feat_dim * 3), nn.ReLU(),
            #nn.Linear(feat_dim * 3, feat_dim * 2), nn.ReLU(),
            nn.Linear(feat_dim * 2, feat_dim), nn.ReLU(),
            nn.Linear(feat_dim, feat_dim), nn.ReLU(),
            nn.Linear(feat_dim, feat_dim)
        )

    #def forward(self, aa, res_nb, chain_nb, pos_atoms, mask_atoms, fragment_type, esm_res_feat,structure_mask=None, sequence_mask=None):
    def forward(self, aa, res_nb, chain_nb, pos_atoms, mask_atoms, fragment_type, structure_mask=None, sequence_mask=None):
        ...
        Args:
            ...
            esm_res_feat:(N,L,1280) huyue from esm model
        """
        N, L = aa.size()
        ...
        #huyue
        #out_feat = self.mlp(torch.cat([aa_feat, crd_feat, dihed_feat, type_feat,esm_res_feat], dim=-1)) # (N, L, F)
