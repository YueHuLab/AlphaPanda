...
class PaddingCollate(object):
    ...
    def _pad_last(x, n, value=0):
        if isinstance(x, torch.Tensor):
            assert x.size(0) <= n
            if x.size(0) == n:
                if x.dim()==3 and x.size(1)==n:
                    return x
                elif x.dim()==3 and x.size(1)<n and x.size(2) == 660:
                    pad_size =[n]+[n - x.size(1)] + list(x.shape[2:])
                    #pad_size=[n]+[n-d.size(1)]+ list(d.shape[2:])
                ...
                elif x.dim()==3 and x.size(1)<n and x.size(2) ==660: # only attentions huyue
                    pad_tm=torch.cat([x, pad], dim=0)
                    pad_size2 =[n] + [n - pad_tm.size(1)] + list(pad_tm.shape[2:])
                    pad2 = torch.full(pad_size2, fill_value=value).to(pad_tm)
                    return torch.cat([pad_tm, pad2], dim=1)
                else: 
                    return torch.cat([x, pad], dim=0)
