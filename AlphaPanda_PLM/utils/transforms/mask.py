...
class MaskSingleCDR(object):
    ...
    def __init__(self, selection=None, augmentation=True):
        ...
        cdr_str_to_enum = {
            ...
            'CDR1': 'CDR1',     # H1 first, then fallback to L1
            'CDR2': 'CDR2',     # H2 first, then fallback to L2
        }
    ...
    def perform_masking_(self, data, selection=None):
        ...
        if selection is None or selection not in cdr_all: #huyue
    ...
    def __call__(self, structure):
        ...
        elif self.selection == 'CDR2':
            if structure['heavy'] is not None:
                data_to_mask = structure['heavy']
                sel = constants.CDR.H2
            else:
                data_to_mask = structure['light']
                sel = constants.CDR.L2
        elif self.selection == 'CDR1':
            if structure['heavy'] is not None:
                data_to_mask = structure['heavy']
                sel = constants.CDR.H1
            else:
                data_to_mask = structure['light']
                sel = constants.CDR.L1
