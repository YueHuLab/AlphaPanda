...
#huyue
#from diffab.modules.alphafold2_pytorch.alphafold2 import Evoformer
from diffab.modules.alphafold2_pytorch.alphafold2 import PairwiseAttentionBlock
from diffab.modules.alphafold2_pytorch.alphafold2 import FeedForward
from einops import rearrange, reduce
from diffab.modules.GeoTrans.GeometricTransformer import GeoTransformer #huyue
from diffab.modules.dcnn.seq_des.util.voxelize import voxelize #huyue
import diffab.modules.dcnn.seq_des.util.canonicalize as canonicalize
import diffab.modules.dcnn.seq_des.models as TreeCNNmodels
import diffab.modules.dcnn.common.atoms
      ...
      #  dim_head = 64,
       # heads = 8, # huyue 
        ...
        self.eps_crd_net = nn.Sequential(
            ...
            #1108
            nn.Linear(self.nf * 4 +3, res_feat_dim * 2 ), nn.ReLU(),
            nn.Linear(res_feat_dim * 2 , res_feat_dim), nn.ReLU(),
        ...
        #self.TreeCNN=TreeCNNmodels.seqPred(nic=len(diffab.modules.dcnn.common.atoms.atoms) + 1 + 21, nf=64, momentum=0.01)
    ...
    #    print(mask_geo_00)
      ...
      #  print("mask_geo_00 \n")
       ...
       # print("\n")
        ...
        t_embed4=t_embed.reshape(N,L,3)
        res_feat4=res_feat.reshape(N,L,self.res_feat_dim)
        in_feat4 = torch.cat([res_feat4, t_embed4], dim=-1 )
        #23-1108
        TreeOut5=TreeOut.reshape(N,L,self.nf * 4)
        t_embed5=t_embed.reshape(N,L,3)

        in_feat5 = torch.cat([TreeOut5, t_embed3], dim=-1 )
        ...
        #1105
        eps_crd = self.eps_crd_net(in_feat5)    # (N, L, 3)
       ...
       #orientation 0823
        ...
        eps_rot = self.eps_rot_net(in_feat5)    # (N, L, 3)
        ...
        #c_denoised = self.eps_seq_net(in_feat4)  # Already softmax-ed, (N, L, 20)
        #1108
        c_denoised = self.eps_seq_net(in_feat5)  # Already softmax-ed, (N, L, 20)
